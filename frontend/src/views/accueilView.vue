<template>
  <div id="button-global">
    ...
    <button @click="deconnecter" class="btn btn-primary">Déconnexion</button>
  </div>
  <div class="container">
                  ...
                  <div class="media-body">
                    <h6>
                      <a
                        ...
                        data-target=".forum-content"
                        class="text-body"
                        >Http client post raw content</a
                      >
                    </h6>
                    <p class="text-secondary">
                      lorem ipsum dolor sit amet lorem ipsum dolor sit amet
                  ...
                  <div class="media-body">
                    <h6>
                      <a
                        ...
                        data-target=".forum-content"
                        class="text-body"
                        >Top rated filter not working</a
                      >
                    </h6>
                    <p class="text-secondary">
                      lorem ipsum dolor sit amet lorem ipsum dolor sit amet
        ...
        .then((posts) => {
          for (let post of posts) {
            ...
            fetch(this.apiGetAllComments)
              .then((res) => res.json())
              .then((comments) => {
                const allCommentsOnePost = comments.filter(
                ...
                fetch(`http://localhost:3000/users/getOneUser/${post.userId}`)
                  ...
                  .then((user) => {
                    this.postsArray.push({
                      ...
                      post: post.post,
                      comments: [],
                      numberComments: allCommentsOnePost.length,
                      dateJour: dateJour,
                      dateHeure: dateHeure,
                    });
                  });
    ...
    methods: {
      ...
      buttonComment(post) {
        ...
        fetch(this.apiGetAllComments)
          ...
          .then((comments) => {
            const allCommentsOnePost = comments.filter(
              (el) => el.postId == post.idPost
            );
            for (let allCommentOnePost of allCommentsOnePost) {
              fetch(
                `http://localhost:3000/users/getOneUser/${allCommentOnePost.userId}`
              )
                .then((res) => res.json())
                .then((commentUser) => {
                  post.comments.push({
                    userName: commentUser.userName,
                    comment: allCommentOnePost.comment,
                  });
                });
            }
          });
      ...
      buttonEnvoisComment(post) {
        ...
        const comment = this.recupComment;
        const objetComment = { userId, comment };
